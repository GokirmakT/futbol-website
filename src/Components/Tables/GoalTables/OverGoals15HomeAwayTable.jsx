...
const OverGoals = ({ goalStats, selectedLeague, isMobile, teamLogos, football, playedMatches, getBgColor }) => {  
  ...
  const [orderBy, setOrderBy] = useState("over15Rate");
            ...
            <Table size="small" stickyHeader sx={{borderRadius: 0}}>
              <TableHead sx={{ "& .MuiTableCell-root": { backgroundColor: "#1d1d1d" } }}>
                <TableRow>
                  ...
                  <SortHeader align="center" field="over15Rate" abc="1.5 Üst"></SortHeader>
                  <SortHeader align="center" field="homeOver15Rate" abc="İç Saha"></SortHeader>
                  <SortHeader align="center" field="awayOver15Rate" abc="Dış Saha"></SortHeader>
                </TableRow>
              </TableHead>
              ...
              <TableBody>
                {sortedRows.map(row => (
                  <TableRow key={row.team} sx={{ "&:hover": { backgroundColor: "#2c2c2c" } }}>
                    ...
                    <TableCell align="center" sx={{color: "#000000ff", fontWeight: "bold", backgroundColor: getBgColor(row.over15Rate), pr: isMobile ? 0 : 2, pl: isMobile ? 0 : 2}}>{row.over15Rate.toFixed(0)}%</TableCell>
                    <TableCell align="center" sx={{color: "#000000ff", fontWeight: "bold", backgroundColor: getBgColor(row.homeOver15Rate), pr: isMobile ? 0 : 2, pl: isMobile ? 0 : 2}}>{row.homeOver15Rate.toFixed(0)}%</TableCell>
                    <TableCell align="center" sx={{color: "#000000ff", fontWeight: "bold", backgroundColor: getBgColor(row.awayOver15Rate), pr: isMobile ? 0 : 2, pl: isMobile ? 0 : 2}}>{row.awayOver15Rate.toFixed(0)}%</TableCell>
